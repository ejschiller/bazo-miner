...
func TestTargetHistory(t *testing.T) {
	...
	for cnt := 0; cnt < 10; cnt++ {
		tmpBlock = newBlock(tmpBlock.Hash, [crypto.COMM_KEY_LENGTH_ED]byte{}, tmpBlock.Height+1)
	...
	tmpBlock = newBlock(blocks[len(blocks)-1].Hash, [crypto.COMM_PROOF_LENGTH_ED]byte{}, blocks[len(blocks)-1].Height+1)
	finalizeBlock(tmpBlock)
	validate(tmpBlock, false)
...
func TestTimestamps(t *testing.T) {
	...
	prevHash := [32]byte{}
	for cnt := 0; cnt < 0; cnt++ {
		b := newBlock(prevHash, [crypto.COMM_PROOF_LENGTH_ED]byte{}, 1)
