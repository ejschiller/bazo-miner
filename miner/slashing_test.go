...
func TestSlashingCondition(t *testing.T) {
	...
	forkBlock := newBlock([32]byte{}, [crypto.COMM_PROOF_LENGTH_ED]byte{}, 1)
	...
	// genesis <- forkBlock <- b
	b := newBlock(forkBlock.Hash, [crypto.COMM_PROOF_LENGTH_ED]byte{}, 2)
	...
	b2 := newBlock(forkBlock.Hash, [crypto.COMM_PROOF_LENGTH_ED]byte{}, 2)
	...
	b3 := newBlock(b2.Hash, [crypto.COMM_PROOF_LENGTH_ED]byte{}, 3)
